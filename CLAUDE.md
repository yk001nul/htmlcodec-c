...
- **`htmlcodec-c-lib`** — static library: `html-tokenizer.c`, `html-codec.c`, `css-tokenizer.c`, `css-codec.c`, `nl-en-tokenizer.c`, `nl-en-opt.c`, `nl-en-codec.c`, `cl-javascript-en-tokenizer.c`, `cl-javascript-codec.c`, `nl-en-us-hyphenator.c`, `json-tokenizer.c`, `cmdline.c`. Defines `HTMLCODEC_STATIC` as a PUBLIC compile definition so all targets that link it inherit the correct API macro automatically.
- **`htmlcodec-c-shared`** — shared library (`.dll` / `.so`) built from the same sources. Defines `HTMLCODEC_BUILDING_DLL` privately so `HTMLCODEC_API` expands to `__declspec(dllexport)` on Windows and `__attribute__((visibility("default")))` on GCC/Clang. GCC/Clang builds use `-fvisibility=hidden` so only explicitly annotated symbols are exported.
...
All public headers include `htmlcodec-c.h`, which defines the `HTMLCODEC_API` visibility macro with three cases:

| Macro defined at build time | `HTMLCODEC_API` expands to | Use case |
|---|---|---|
| `HTMLCODEC_BUILDING_DLL` | `__declspec(dllexport)` / `visibility("default")` | Compiling `htmlcodec-c-shared` itself |
| `HTMLCODEC_STATIC` | *(nothing)* | Linking `htmlcodec-c-lib` (propagated automatically via CMake PUBLIC define) |
| neither | `__declspec(dllimport)` / `visibility("default")` | External consumer of the shared library |

Every public function declaration and every `extern const` variable declaration in the 12 public headers is annotated with `HTMLCODEC_API`. All headers also wrap their declarations in `#ifdef __cplusplus extern "C" { #endif` guards for C++ / Python pybind11 consumers.

**Thread safety:** All lazy-initialisation in the library is thread-safe (see `hc-once.h` below). No consumer-side warm-up calls are required.

### Internal Helpers

- **`hc-once.h`** — portable one-time initialisation header (not installed, not part of the public API). Provides `hc_once_t` / `HC_ONCE_INIT` / `hc_call_once(flag, fn)`. On Windows uses `volatile LONG` + `InterlockedCompareExchange`/`InterlockedExchange` + `Sleep(0)` spin (no `<stdatomic.h>` required, compatible with all MSVC versions targeting Vista+). On POSIX/Android uses C11 `<stdatomic.h>` with `atomic_flag` (test-and-set) + `atomic_int` (done flag) and acquire/release ordering. The init function is called exactly once across all threads; concurrent callers spin until it completes. Used by `nl-en-tokenizer.c`, `cl-javascript-en-tokenizer.c`, `html-codec.c`, and `nl-en-us-hyphenator.c`.
